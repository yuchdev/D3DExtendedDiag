...
// Global variables
//-----------------------------------------------------------------------------
std::unique_ptr<CDxDiagInfo> g_pDxDiagInfo;
                    ...
                    LPSTR lpCmdLine, INT nCmdShow )
{
    ...
    g_pDxDiagInfo.reset(new CDxDiagInfo());

    HRESULT hr = g_pDxDiagInfo->Init( TRUE );
    if( FAILED( hr ) )
    {
    ...
    hr = g_pDxDiagInfo->QueryDxDiagViaDll();
    if( FAILED( hr ) )
    {
...
VOID FillListBoxWithLogicalDiskInfo( HWND hwndList )
{
    CDxDiagInfo* pDxDiag = g_pDxDiagInfo.get();
    TCHAR szTmp[3000];
    TCHAR szName[3000];
