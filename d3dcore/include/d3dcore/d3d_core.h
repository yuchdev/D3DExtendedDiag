...
public:
    ...
    /// @brief Callback function called for every container and every property of Direct3D
    /// callback(const wchar_t* container_name, const wchar_t* property_name, const std::wstring& property_value)
    /// "container_name" is hierarchical path, where child containers are separated with "." symbol
    /// "property_name" is a single property name
    /// "property_value" is a value string representation
    using traverse_callback_t = void(*)(const wchar_t*, const wchar_t*, const std::wstring&);
    ...
    /// @param callback: function with the following prototype 
    /// /// callback(const wchar_t* container_name, const wchar_t* property_name, const std::wstring& property_value)
    /// "container_name" is hierarchical path, where child containers are separated with "." symbol
    /// "property_name" is a single property name
    /// "property_value" is a value string representation
